...
{block name="itemLink"}
  {if $item['url']}
    <a href="{$item['url']}"  class="{$item['class']|default:''}" {if $linkTarget || $item['linkTarget']} target="{if $item['linkTarget']}{$item['linkTarget']}{else}{$linkTarget}{/if}"{/if} data-role="button" data-mini="true" data-icon="arrow-r" data-iconpos="right" data-theme="a">
  {else}
    <span class="nolink">
  {/if}
    {if $item['img']}
